...
Herda de `GitPluginBase`, exportado via MEF. Construtor `: base(false)` → **sem** diálogo de settings em Configurações → Plugins. Ver [[🔌 ZimerfeldLfsPlugin]].
...
### 🪟 `LfsForm` — a janela
WinForms `FixedSingle`, não-maximizável, `CenterScreen`, Segoe UI 9. De cima para baixo: banner de patrocínio, dropdown de repo independente + branch, `TabControl` de 3 abas, console de log escuro, painel inferior (Debug + Idioma + Fechar), status strip. Ver [[🪟 LfsForm]].

### ⚡ `LfsService` — executor git/git-lfs
`RunGit(args)` roda `git` num `ProcessStartInfo` (redirecionando stdout/stderr, UTF-8, sem janela) e devolve um `GitResult(StdOut, StdErr, ExitCode)` (record struct; `Ok` = exit 0; `Combined` = stdout+stderr aparado). Métodos de estado, das 3 etapas, e o helper estático `GetRepositoriesFromSettings`. Ver [[⚡ LfsService]].

## 🔗 Desacoplamento do host

> [!important] O plugin assina **NENHUM** evento do host
> Ao contrário do irmão CommitMsg (que ouve `PostRepositoryChanged`), o ZimerfeldLFS é **totalmente desacoplado**: a janela escolhe o repositório **exclusivamente** pelo próprio `cboRepo`. O working dir do host é usado **uma única vez**, como valor pré-selecionado ao abrir a janela. `Register` só guarda o `_commands` para poder abrir os diálogos nativos; não há re-vinculação nem `Application.Idle`. Ver [[📁 Diretório de trabalho independente]].
