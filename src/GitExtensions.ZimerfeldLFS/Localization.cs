// Localization.cs — runtime language dictionaries for the ZimerfeldLFS window
...
/// the user's language choice. The choice is read from disk only once, at process start; thereafter
/// it is driven in-memory by the Language dropdown.
/// </summary>
public static class I18n
{
    private static readonly string ConfigPath = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "GitExtensions", "ZimerfeldLFS.language.json");
    ...
    /// falling back to en-US then to an empty map (in which case <see cref="Translator"/> echoes each
    /// key back).
    /// </summary>
    public static Translator Load(string scope) => Load(scope, _current);

    /// <summary>Same as <see cref="Load(string)"/> but for an explicit <paramref name="lang"/>.</summary>
    ...
    private static Dictionary<string, string>? ReadEmbedded(string fileName)
    {
        try
        {
            string logicalName = $"GitExtensions.ZimerfeldLFS.Resources.{fileName}";
...
/// <summary>Holds the loaded strings; returns the key itself when a key is missing.</summary>
