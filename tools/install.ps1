...
<#
.SYNOPSIS
    Instala o plugin ZimerfeldLFS no GitExtensions.

.DESCRIPTION
    Pode ser executado de duas formas:

      Opcao A - standalone (Execute diretamente):
          cd C:\GitExtensions\GitExtensions.ZimerfeldLFS\tools
          .\install.ps1

      Opcao B - via NuGet Package Manager Console (Visual Studio):
          Install-Package GitExtensions.ZimerfeldLFS -Source C:\NUGET
...
$dllName = "GitExtensions.Plugins.ZimerfeldLFS.dll"
$dll     = Join-Path $dllDir $dllName

if (-not (Test-Path $dll)) {
    Write-Error ("DLL nao encontrada: $dll`n`nExecute build.ps1 primeiro para compilar o plugin:`n  pwsh C:\GitExtensions\GitExtensions.ZimerfeldLFS\build.ps1")
    exit 1
}
...
try {
    ...
    Write-Host ""
    Write-Host "Reinicie o GitExtensions e acesse Plugins -> ZimerfeldLFS."
}
catch {
